...
class C01Simple {
	...
	// {} 내에서 생성, 벗어나면 소멸
	// 메서드 호출 시마다 프레임(Frame)이라는 작은 메모리 블록이 생성되어 메서드의 지역 변수와 연산 중 발생한 메서드가
	// 종료되면 해당 프레임이 스택에서 제거됨.
	...
	// 3. 클래스영역(== 메서드 영역)  : 공유메모리, static변수, 일반 메서드, 생성자 메서드, 클래스 정보, 상수
	...
	public class MyClass {
		...
		public static void a_method() {
			System.out.println("This is a stati method");
		}
	...
	// 반면에 비 정적 멤버는 개체의 인스턴스에 속하며, 해당 클래스의 객체를 생성한 후에만 접근할 수 있음.
