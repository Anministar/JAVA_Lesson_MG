...
// 2. 메서드에게 예외를 떠넘기기 (rethrowing)		: 메서드에서 예외를 처리하지 않고, 해당 예외를 다시 호출한 메서드에게 떠 넘기는 방식
...
//			int value = -5;
//			if (value < 0) {
//				// 여기에서 throw문을 사용하여 IllegalArgumentException을 명시적으로 던짐.
...
//			System.out.println("Exception caught int someMethod : " + e.getMessage());
...
// 따라서 methodA를 호출한 main 메서드에서 에외를 처리하게 됨.

// ==> 이런식으로 예외는 호출 스택에 따라 올라가면서 에외를 처리할 수 있는 첫번째 catch 블록이 있는 곳에서 처리되거나,
