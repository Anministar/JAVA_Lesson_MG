...
public class C02StringClass {
	public static void main(String[] args) {
		...
		// 이건 자바의 메모리 구조상 JVM이 최적화를 시키기 때문에 "java"가 상수 풀(pool) 영역에 있다면 그걸 동일시하게 만드는
		// 구조를 가지고 있죠.
		// "java"가 실제 객체와 동일한 hashCode를 지니게 된다.
		...
		// 기본적으로는 메모리 주소가 아닌 객체의 내용에 따라 해시 코드를 생성하는데, 이는 equals 메서드와 함께 사용되어 컬렉션 등에서 객체를 식별하는 데 활용
		...
		// 1과 2의 저장 방식이 다름
		// "=="은 서로 동일한 인스턴스냐, 즉 같은 메모리 주소값을 지니나, 객체가 참조하고 있는 메모리 '값'을 비교
